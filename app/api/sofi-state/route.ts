...
async function ensureSchema(sql: Sql) {
  ...
  await sql`
    create table if not exists sofi_cash_flow_transactions (
      id text primary key,
...
export async function GET() {
  ...
  const transactions = await sql`
    select
      ...
      paid,
      notes
    from sofi_cash_flow_transactions
    order by transaction_date desc, created_at desc
  `;

  const state: SofiState = {
...
export async function PUT(request: Request) {
  ...
  const state = await request.json() as SofiState;

  await ensureSchema(sql);
  await sql`delete from sofi_cash_flow_transactions`;

  for (const transaction of state.transactions) {
    await sql`
      insert into sofi_cash_flow_transactions (
        id,
        transaction_date,
        merchant,
        amount,
